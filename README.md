...
2. **定型文の自動入力 (Predefined String)**:
   パネルを開いた際、「動画を要約する」などのよく使うフレーズを自動で入力します。自動入力は同一動画で初回のみ動作します。2回目以降は入力エリアを空の状態で開くため、追加の質問をすぐに手入力できます。

3. **自動送信 (Auto-submit)**:
   設定をオンにすると、「質問する」ボタンを押した瞬間に、自動入力されたフレーズを自動的に送信します。動画ごとに1クリックで要約や質問を送信できます。自動入力と同様、自動送信も同一動画で初回のみ動作します。
...
3. 設定画面で以下のカスタマイズが可能です：
   - **Predefined String**: パネルが開いたときに自動入力される文字列です（デフォルト: `動画を要約する`）。同一動画での自動入力は初回のみです。
   - **Auto-submit**: チェックを入れると、パネルが開いた瞬間に自動で質問が送信されます。同一動画での自動入力・自動送信はいずれも初回のみです。
...
3. **厳格なクリックトラッキングによる意図しない自動送信の防止**
   ...
   - 同一動画に対する自動入力・自動送信はそれぞれ `autoFilledVideoId` と `autoSubmittedVideoId` で追跡し、初回のみ実行されます。2 回目以降のクリックでは自動入力も自動送信も行わず、ユーザーが追加の質問を手入力できるクリーンな状態でパネルを開きます。これらのフラグは別動画への遷移時やページ離脱時にリセットされます。
